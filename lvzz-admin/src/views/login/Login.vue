  ...
  <div id="login">
    ...
    <div class="wrap-banner">
      <div class="main-title" v-show="showTitle">
        <h2>旅游自助管理平台后台登录</h2>
        <p class="subtitle">Welcome to lvzz</p>
        <p class="subtitle">By Admin</p>
        <br />
...
<h2 style="color: white;">注册</h2>
  <el-form :model="regForm" ref="regForm" :rules="Rrules">
    <el-form-item  prop="name">
      <el-input v-model="regForm.name" placeholder="请输入用户名" @blur="cheakUsername()" autocomplete="off"></el-input>
    </el-form-item>
    <el-form-item prop="password">
...
export default {
  ...
  data() {
    return {
      ...
      vcUrl: this.path+"/admin/verifyCode?time=" + new Date(),
  ...
  methods: {
    ...
    updateVerifyCode() {
      this.vcUrl = this.path+"/admin/verifyCode?time=" + new Date();
    },
    login(formName){
         const _this=this;
      ...
      params.append("code", this.loginForm.code);
       this.$refs[formName].validate((valid) => {
          if (valid) {
            this.$http.post('/admin/queryAdmin',params).then(res=>{
            if (res.data.code === 3) {
    ...
    reg(formName){
          ...
          if (valid) {
              ...
              if(this.regForm.password == this.regForm.repassword){
               this.$http.post('/admin/addAdmin',this.regForm).then(res=>{
    ...
    cheakUsername(){
      let name=this.regForm.name || null
      this.$http.get('/admin/cheakAdminName/'+name).then(res=>{
        if(res.data.code==200){
           this.isOk=true;
        }else{
